...
@implementation NSObject (HJWKeyValue)
#pragma mark - 公有方法
#pragma mark - 字典转模型
/**
 *  通过JSON数据来创建一个模型
...
{
    HJWAssertParamNotNil2(data, nil);
...
+ (instancetype)objectWithKeyValues:(NSDictionary *)keyValues
{
    ...
    HJWAssert2([keyValues isKindOfClass:[NSDictionary class]], desc, nil);
...
{
    HJWAssertParamNotNil2(filename, nil);
...
{
    HJWAssertParamNotNil2(file, nil);
...
{
    NSString *desc = [NSString stringWithFormat:@"keyValues is not a NSDictionary - keyValues参数不是一个字典, keyValues is a %@ - keyValues参数是一个%@", keyValues.class, keyValues.class];
    HJWAssert2([keyValues isKindOfClass:[NSDictionary class]], desc, );
...
+ (NSArray *)keyValuesArrayWithObjectArray:(NSArray *)objectArray
{
    ...
    HJWAssert2([objectArray isKindOfClass:[NSArray class]], desc, nil);
...
+ (NSArray *)objectArrayWithKeyValuesArray:(NSArray *)keyValuesArray
{
    ...
    HJWAssert2([keyValuesArray isKindOfClass:[NSArray class]], desc, nil);
...
{
    HJWAssertParamNotNil2(propertyName, nil);
    NSString *key = nil;
    // 1.查看有没有需要替换的key
